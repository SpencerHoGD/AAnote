...
def cleanFile(df):
    ...
    colsStrIndex = [5, 6, 10, 19, 20, 22, 23]
    colsValueIndex = set(list(range(len(columnsList)))[2:]).difference(set(colsValueIndex))
    # colsHavevalue = [~dn.columns.isin([i for i in colsHavestr])]
    print(colsValueIndex)
    # print(colsHavevalue)
    # dn.iloc[:, [x for x in colsHavevalue]] = dn.iloc[:, [x for x in colsHavevalue]].apply(pd.to_numeric, errors='coerce', downcast='float')

    # # 处理有亿万等字眼的列，去除空格
    # collist = [x for x in dn.columns]
    # colsHavestr = [5, 6, 10, 19, 20, 22, 23]    #有亿万字眼的列
    # cols3 = []
    # for colIndex in colsHavestr:
