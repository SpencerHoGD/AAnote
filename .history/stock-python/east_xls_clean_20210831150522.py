...
# dn['现量'] = dn['现量'].str.strip()
# print(dn.现量)
colist = [i for i in dn.columns]
print(colist)
# cols3 = ['l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', 'l', ]
# cols2 = [5, 6, 11, 21, 23, 24, 26, 27, 29, 30, 31, 32]
# for x in cols2:
    # dn.iloc[:, [x]] = dn.iloc[:, [x]].str.strip()   #yebuxing
    
# dn.iloc[:, [x for x in cols2]] = dn.iloc[:, [x for x in cols2]].str.strip()  #要一列一列来
# dn.iloc[:, [5]] = dn.iloc[:, [5]].apply(str2value)

# dn.iloc[:, [x for x in cols2]] = dn.iloc[:, [x for x in cols2]].apply(pd.to_numeric, errors='ignore')
# dn.iloc[:, [x for x in cols2]] = dn.iloc[:, [x for x in cols2]].apply(pd.to_numeric)
