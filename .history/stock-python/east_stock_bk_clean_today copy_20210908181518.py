...
def cleanFile(df):
    ...
    # for column in dn[: [x for x in range(len(dn.columns()))]]:
        # pass

    a_list = [x for x in range(len(dn.columns))]
    # a_list = [x for x, y in enumerate(dn.columns)]
    for i in a_list:
        dn.iloc[: [i]] = dn.iloc[: [i]].str.strip()
    # return a_list
    # dn['名称'] = dn['名称'].str.strip()
    # dn['领涨股'] = dn['领涨股'].str.strip()
    # dn['涨跌家数'] = dn['涨跌家数'].str.strip()
    # dn['涨跌比'] = dn['涨跌比'].str.strip()
